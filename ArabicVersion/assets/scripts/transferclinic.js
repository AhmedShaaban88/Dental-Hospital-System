...
const fromClinicName = $("#fromClinicName");
const fromClinicNameHelper = $("#fromClinicNameHelper");
const toClinicName = $("#toClinicName");
const toClinicNameHelper = $("#toClinicNameHelper");
...
fromClinicName.on("change", function () {
    if(!fromClinicNameHelper.hasClass('hidden')){
        fromClinicNameHelper.addClass("hidden");
    }else if($(this).val() === ''){
        fromClinicNameHelper.removeClass("hidden");
    }
});
toClinicName.on("change", function () {
    if(!toClinicNameHelper.hasClass('hidden')){
        toClinicNameHelper.addClass("hidden");
    }else if($(this).val() === ''){
        toClinicNameHelper.removeClass("hidden");
    }
});
...
transferClinicForm.on("submit", function (e) {
    ...
    else if(fromClinicName.val() === ''){
        fromClinicNameHelper.removeClass('hidden');
        e.preventDefault();
    }
    else if(toClinicName.val() === ''){
        toClinicNameHelper.removeClass('hidden');
        e.preventDefault();
    }
});
$(document).ready(function () {
    $(".clear-select").on("click", function () {
        ...
        $(this).next("select").next(".select2").find(".select2-selection__rendered").html('');
